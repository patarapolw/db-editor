...
import dbEditorHtml from "./db-editor.html";
import "./db-editor.scss";
...
export class DbEditor {
    ...
    constructor(settings: IDbEditorSettings) {
            ...
            switch (col.type) {
                ...
                case "datetime":
                    ...
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label">${toTitle(col.name)}</label>
                        <div class="col-sm-10">
        ...
        this.$el.mdEditor = $("<div><textarea></textarea></div>");
    ...
    private async addEntry(entry: any, isNew = false) {
        ...
        for (const col of this.settings.columns) {
            ...
            if (col.type === "datetime") {
                const $input = $(`<input class="db-editor-clear" value="${data || ""}">`);
                $seg.html("");
                $seg.append($input);
    ...
    private setPageNav() {
        ...
        (this.$el.prevAll.get(0) as HTMLButtonElement).disabled =
        (this.$el.prev.get(0) as HTMLButtonElement).disabled = !(this.page.from > 1);

        (this.$el.nextAll.get(0) as HTMLButtonElement).disabled =
        (this.$el.next.get(0) as HTMLButtonElement).disabled = !(this.page.to < this.page.total);
    }
